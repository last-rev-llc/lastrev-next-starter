...
const PageGeneral = ({ modules, pageTitle, pageHero, moduleLookup }) => {
  return (
    <div className={styles.pageGeneral}>
      {pageHero ? (
        <div data-testid="PageGeneral-moduleHero">
          <ModuleHero className={styles.moduleHero} {...pageHero} />
        </div>
      ) : (
      ...
      {modules && modules.length ? (
        <div data-testid="PageGeneral-modules">
          {modules.map((module) => (
            <ContentModule key={module.sys.id} moduleLookup={moduleLookup} {...module} />
          ))}
        </div>
      ) : null}
    </div>
